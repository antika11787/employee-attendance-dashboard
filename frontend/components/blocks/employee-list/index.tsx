...
const EmployeeList = () => {
    ...
    const loadMoreEmployees = () => {
        // Load additional 20 employees when load more button is clicked
        const currentlyDisplayed = displayedEmployees.length;
        const nextBatch = data.slice(currentlyDisplayed, currentlyDisplayed + 20);
        setDisplayedEmployees(prevEmployees => [...prevEmployees, ...nextBatch]);
    };

    const showLessEmployees = () => {
        // Remove the last 20 displayed employees
        setDisplayedEmployees(prevEmployees => prevEmployees.slice(0, prevEmployees.length - 20));
    };
        ...
        <div className='employee-list-page'>
            ...
            <div className='employee-list-wrapper'>
                <div className='employee-list-container'>
                    ...
                    {data.length > displayedEmployees.length && (
                        <button className="load-more-button" onClick={loadMoreEmployees}>Load More</button>
                    )}
                    {displayedEmployees.length > 20 && (
                        <button className="show-less-button" onClick={showLessEmployees}>Show Less</button>
                    )}
                    </div>
                </div>
            </div>
